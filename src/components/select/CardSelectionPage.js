...
function CardSelectionPage() {
  ...
  const CARDS_PER_ROW = TOTAL_CARDS / ROW_COUNT; // 78/3 = 26
    ...
    setTimeout(() => {
      ...
      setCards((prevCards) =>
        prevCards.map((card, index) => {
          const col = index % CARDS_PER_ROW; // 0~25
          const row = Math.floor(index / CARDS_PER_ROW); // 0~2
          const centerOffset = (CARDS_PER_ROW * 75) / 2; // 975/2 = 487.5

          return {
            ...card,
            left: `calc(50% + ${col * 3 - 40}%)`,
            top: `calc(50% + ${row * 20 - 30}%)`,
  ...
  return (
    <div className="selection-container">
      ...
      <div
        className={`card-container ${showLightEffect ? "light-expand" : ""}`}
      >
        {cards.map((card) => (
          <img
            key={card.id}
            ...
            style={{
              ...
              transition:
                "left 1s ease-in-out, top 1s ease-in-out, transform 0.5s ease-in-out",
            }}
